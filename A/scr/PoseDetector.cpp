    ...
    const BodyPose& pose) const
{
    ...
    // -------------------------
    // 关键点。名称和 confidence 属于调试信息，不放在用户画面中。
    // -------------------------
    for (const auto& item :
        pose.joints)
    {
    ...
    if (!mainPersonConfirmed_)
    {
        ...
        const int helpY = std::max(30, frame.rows - 42);
        cv::putText(frame, "SPACE: confirm   P: push-up   S: squat",
            cv::Point(20, helpY), cv::FONT_HERSHEY_SIMPLEX, 0.5,
            cv::Scalar(255, 255, 255), 1, cv::LINE_AA);
        cv::putText(frame, "R: reselect   C: clear   ESC: exit",
            cv::Point(20, std::max(30, frame.rows - 18)),
    ...
    else
    {
        ...
        const int helpY = std::max(30, frame.rows - 42);
        cv::putText(frame, "P: push-up   S: squat   C: clear",
            cv::Point(20, helpY), cv::FONT_HERSHEY_SIMPLEX, 0.5,
            cv::Scalar(255, 255, 255), 1, cv::LINE_AA);
        cv::putText(frame, "R: reselect   ESC: exit",
            cv::Point(20, std::max(30, frame.rows - 18)),
            cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255, 255, 255),
            1, cv::LINE_AA);
}
